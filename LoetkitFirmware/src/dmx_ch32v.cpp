...
unsigned char dmx_getValue(unsigned short chan)
{
    if (chan > 511) return 0; // out of range
    return dmx_data[chan];
}

void dmx_getValues(unsigned short startchan, unsigned char *p, unsigned short len)
{
    for (unsigned short i = 0; i < len; i++)
    {
        p[i] = dmx_data[(startchan + i) & 0x1FF];
    }
}
